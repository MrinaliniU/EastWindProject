...
public class NextGreaterElement1 {
    /*
        Runtime: 4 ms, faster than 70.98%
        Memory Usage: 40.2 MB, less than 7.41%
     */
    public int[] nextGreaterElement(int[] nums1, int[] nums2) {
        Stack<Integer> monotonic_stack = new Stack<>();
        HashMap<Integer, Integer> nextGreaterElement_map = new HashMap<>();
        for(int i = 0; i < nums2.length; i++){
            if(!monotonic_stack.isEmpty() && monotonic_stack.peek() < nums2[i]){
                while(!monotonic_stack.isEmpty() && monotonic_stack.peek() < nums2[i])
                    nextGreaterElement_map.put(monotonic_stack.pop(), nums2[i]);
            }
            monotonic_stack.add(nums2[i]);
            if(i == nums2.length - 1) {
                while(!monotonic_stack.isEmpty()) nextGreaterElement_map.put(monotonic_stack.pop(), -1);
            }
        }
        for(int i = 0; i < nums1.length; i++){
            nums1[i] = nextGreaterElement_map.get(nums1[i]);
        }
        return nums1;
    }
    ...
    public static void main(String[] args){
        NextGreaterElement1 n1 = new NextGreaterElement1();
        ...
        System.out.println(Arrays.toString(n1.nextGreaterElement(nums1, nums2)));
    }
